...
outdir = "Converted" # Output folder for the converted data 

trial_data_dir = os.path.join(outdir, "frames") # Output subfolder for metadata
...
for f in Path(rawdir).rglob("*.cfs"):
    ...
    basename = f.stem
    trial_outpath = os.path.join(trial_data_dir, basename + "_frames.csv")
